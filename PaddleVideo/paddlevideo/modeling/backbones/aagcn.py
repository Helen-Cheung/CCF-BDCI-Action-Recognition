...
class unit_gcn(nn.Layer):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3, adaptive=True, attention=True):
        ...
        for i in range(self.num_subset):
            self.conv_d.append(nn.Conv2D(in_channels, out_channels, 1, weight_attr=nn.initializer.KaimingNormal()))

        if adaptive:
            self.pa = paddle.to_tensor(A.astype(np.float32))
            self.PA = paddle.create_parameter(self.pa.shape, self.pa.dtype)
            #self.PA = nn.Parameter(torch.from_numpy(A.astype(np.float32)))
            self.alp = paddle.zeros([1])
            self.alpha = paddle.create_parameter(self.alp.shape, self.alp.dtype)
            ...
            for i in range(self.num_subset):
                self.conv_a.append(nn.Conv2D(in_channels, inter_channels, 1, weight_attr=nn.initializer.KaimingNormal()))
                self.conv_b.append(nn.Conv2D(in_channels, inter_channels, 1, weight_attr=nn.initializer.KaimingNormal()))
        else:
            self.a = paddle.to_tensor(A.astype(np.float32))
            self.A = paddle.create_parameter(self.a.shape, self.a.dtype)
            #self.A = Variable(torch.from_numpy(A.astype(np.float32)), requires_grad=False)
        ...
        if attention:
            ...
            # temporal attention
            self.conv_ta = nn.Conv1D(out_channels, 1, 9, padding=4, weight_attr=nn.initializer.Constant(0), bias_attr=nn.initializer.Constant(0))
            ...
            pad = (ker_jpt - 1) // 2
            self.conv_sa = nn.Conv1D(out_channels, 1, ker_jpt, padding=pad , weight_attr=nn.initializer.XavierNormal(), bias_attr=nn.initializer.Constant(0))
            #nn.init.xavier_normal_(self.conv_sa.weight)
            #nn.init.constant_(self.conv_sa.bias, 0)

            # channel attention
            rr = 2
            self.fc1c = nn.Linear(out_channels, out_channels // rr, weight_attr=nn.initializer.KaimingNormal(), bias_attr=nn.initializer.Constant(0))
            self.fc2c = nn.Linear(out_channels // rr, out_channels, weight_attr=nn.initializer.Constant(0), bias_attr=nn.initializer.Constant(0))
...
class AAGCN(nn.Layer):
    def __init__(self, num_point=25, num_person=1, graph=None, graph_args=dict(), in_channels=2,
                 drop_out=0, adaptive=True, attention=True,layout='fsd10',
                 strategy='spatial'):
        ...
        #self.fc = nn.Linear(256, num_class, weight_attr= nn.initializer.Normal(0.0, math.sqrt(2. / num_class))
